...
router.post('/admin/updatePassword', function (req, res, next) {
    var obj = req.body;
    var sqlUp = "update admin_user set password = '"+ obj.password +"' where account = 'dwdadmin'";
    var sql = "SELECT * FROM admin_user WHERE account = 'dwdadmin'";
    poolUser.getConnection(function (err, conn) {
        if (err) console.log("POOL updatePassword==> " + err);
        conn.query(sql, function (errr,result) {
            if (errr) {
                res.json({message: '修改失败', code: 1 });
                conn.release();
            ...
            var objResult = result ? result[0] : {};
            if (obj.oldPassword !== objResult.password) {
                res.json({message: '旧密码错误', code: 1 });
                conn.release();
                return
            }
            conn.query(sqlUp, function (errU,result) {
                if (errU) {
                    res.json({message: '修改失败', code: 1 });
                } else {
                    res.json({message: '修改成功', code: 0 });
