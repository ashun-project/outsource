<template>
    <div class="rechargetype">
        <my-header title="充值类型" bkUrl="/my"></my-header>
        <div class="user-info">
            <div class="toxiang">
                <!-- <i class="iconfont">&#xe666;</i> -->
        ...
        <div class="cell-list">
            <ul>
                <li v-if="pays.banks.length > 0" :class="{active: active == 1}">
                    <div class="bank-type pay-type" @click="showChild(1)">
                        <p>
                            <i class="iconfont">&#xe68e;</i>
                            <span>银行卡充值</span>
                        </p>
                    ...
                    <div class="bank-list" v-for="item in pays.banks" :key="item.id" @click="goRoute(item.id)">
                        <span>{{ item.bank_name }}</span>
                        <i class="iconfont">&#xe6e0;</i>
                    </div>
                </li>
                <li v-if="pays.weixin.length > 0" :class="{active: active == 2}">
                    <div class="weixin-type pay-type" @click="showChild(2)">
                        <p>
                            <i class="iconfont">&#xe607;</i>
                            <span>微信充值</span>
                        </p>
                    ...
                    <div class="bank-list" v-for="item in pays.weixin" :key="item.id" @click="goRoute(item.id)">
                        <span>{{ item.account_name }}</span>
                        <i class="iconfont">&#xe6e0;</i>
                    </div>
                </li>
                <li v-if="pays.zhifubao.length > 0" :class="{active: active == 3}">
                    <div class="zhifubao-type pay-type" @click="showChild(3)">
                        <p>
                            <i class="iconfont">&#xe60a;</i>
                            <span>支付宝充值</span>
                        </p>
                    ...
                    <div class="bank-list" v-for="item in pays.zhifubao" :key="item.id" @click="goRoute(item.id)">
                        <span>{{ item.account_name }}</span>
                        <i class="iconfont">&#xe6e0;</i>
                    </div>
                </li>
...
export default {
    ...
    methods: {
        goRoute(id) {
            this.$router.push('/rechargebank/' + id)
        },
        showChild(num) {
            if (this.active == num) {
                num = 0;
        ...
        getBanks() {
            this.$http.post('/grab/banks', {group_type: this.user.group_type}).then(response => {
                var data = response.banks || [];
                var banks = data.filter(item => item.type === 1);
                var weixin = data.filter(item => item.type === 2);
                var zhifubao = data.filter(item => item.type === 3);
                
                this.$set(this.pays, 'banks', banks);
                this.$set(this.pays, 'weixin', weixin);
                this.$set(this.pays, 'zhifubao', zhifubao);
            }).catch(() => {})
        }
    }
}
...
.cell-list li.active .pay-type > i{
    display: block;
...
.cell-list li .weixin-type p i{
    color: #00c784;
}
.cell-list li .zhifubao-type p i{
    color: #09b4fd;
}
.cell-list li .bank-list{
    margin-left: 22px;
    line-height: 40px;
...
.cell-list li .bank-list:active{
    background: #faeaea;
}
.cell-list li .bank-list:last-child{
    border-bottom: 0;
}
.cell-list li .bank-list span{
    font-size: 13px;
}
.cell-list li .bank-list i{
    font-size: 14px;
    float: right;
    color: #666;
}
