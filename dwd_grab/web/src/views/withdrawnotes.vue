<template>
    <div class="withdrawnotes">
        <my-header title="提现记录" bkUrl="/my"></my-header>
        ...
        infinite-scroll-distance="10">
            <li v-for="item in list" :key="item.id" 
                ...
                <div class="info">
                    <span class="title-txt">{{ item.withdraw_type | getType }}&nbsp;{{ item.amount.toFixed(2) }}元</span>
                    <span class="detail-txt">{{ formatDate(item.create_date) }}</span>
                </div>
                <div class="status">
                    <span v-if="item.status === 1">审核中...</span>
                    <span v-else-if="item.status === 2">提现成功</span>
...
export default {
    ...
    methods: {
       loadMore() {
            this.loading = true;
            this.$http.post('/grab/withdrawList', { userId: this.user.id, page: this.page }).then(response => {
        ...
        goRoute(val) {
            this.$router.push('/withdrawdetail/' + val)
        },
        formatDate(date) {
